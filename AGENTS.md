...
Release is scripted via an OpenCode skill at `.opencode/skills/release/SKILL.md`. Invoke with `/release`.
...
`.opencode/skills/release/SKILL.md` — orchestrated release flow: version bump → clean → build → summarize changes → publish to GitHub Release. Invoke with `/release`.
