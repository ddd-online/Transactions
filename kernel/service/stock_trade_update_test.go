...
func TestUpdateTradeTimeOnlyKeepsClearedStockValid(t *testing.T) {
	svc, ws := newStockService(t)

	if _, err := svc.SetPrincipal(ws, testLedgerID, 10000000); err != nil {
		t.Fatalf("设置本金失败: %v", err)
	}
	openTime := int64(1700008000)
	closeTime := int64(1700008100)
	if _, err := svc.CreateTrade(ws, testLedgerID, testCode, testName, models.StockTradeOpen, 1000, 10, openTime, ""); err != nil {
		t.Fatalf("建仓失败: %v", err)
	}
	if _, err := svc.CreateTrade(ws, testLedgerID, testCode, testName, models.StockTradeClose, 1100, 10, closeTime, ""); err != nil {
	...
	// 仅修改建仓的成交时间（移到清仓之后）也允许保存：
	// 持仓按录入顺序重建，成交时间只影响展示与统计时点
	newOpenTime := closeTime + 2000
	updated, err := svc.UpdateTrade(ws, testLedgerID, openID, 1000, 10, newOpenTime)
	if err != nil {
		t.Fatalf("仅修改时间应成功: %v", err)
	}
	if updated.TradeTime != newOpenTime {
		t.Fatalf("成交时间未更新: %+v", updated)
	}
	history, err := svc.GetTradeHistorySummary(ws, testLedgerID)
	if err != nil {
	...
	if overview.RealizedPnl != history.TotalPnl {
		t.Fatalf("账户与历史盈亏应一致: %d / %d", overview.RealizedPnl, history.TotalPnl)
	}
}
